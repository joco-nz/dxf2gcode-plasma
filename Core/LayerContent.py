...
class LayerContentClass:
    ...
    def __init__(self, LayerNr=None, LayerName='', shapes=[]):
        ...
        if self.isParameterizableLayer():
            ...
            lopts_re = re.compile("([a-zA-Z]+ *"+vars.Layer_Options['idfloatseperator']+" *[\-\.0-9]+)")
            print lopts_re.findall(layer_commands)
            for lc in lopts_re.findall(layer_commands):
                name, value = lc.split(vars.Layer_Options['idfloatseperator'])
                name = name.strip()
    ...
    def should_ignore(self):
        return self.LayerName.startswith("IGNORE"+g.config.vars.Layer_Options['idfloatseperator']) or self.isBreakLayer()
        
    def isBreakLayer(self):
        return self.LayerName.startswith("BREAKS"+g.config.vars.Layer_Options['idfloatseperator'])
        
    def isMillLayer(self):
        print '\"%s\"' % ("MILL"+g.config.vars.Layer_Options['idfloatseperator'])
        return self.LayerName.startswith("MILL"+g.config.vars.Layer_Options['idfloatseperator'])
        
    def isDrillLayer(self):
        return self.LayerName.startswith("DRILL"+g.config.vars.Layer_Options['idfloatseperator'])
