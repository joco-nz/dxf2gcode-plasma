                    ...
                    if(NewCurve[-3].r <= NewCurve[-2].r)\
                        ...
                        try:
                            if max(diff) < self.epsilon:
                                tau = self.calc_active_tolerance_inc(self.epsilon,
                                                                     triarc, Arc0, Arc1)
                        ...
                        try:
                            if max(diff) < self.epsilon:
                                tau = self.calc_active_tolerance_dec(self.epsilon,
                                                                     triarc, Arc0, Arc1)
...
##        print Arc1
    ...
    def fit_triac_by_dec_biarc(self, arc, eps):
        ...
        Arc0 = ArcGeo(Pa = arc[0].Pa, Pe = Pn, O = Oa, r = ra,
                      s_ang = Oa.norm_angle(arc[0].Pa),
                      e_ang = Oa.norm_angle(Pn), dir = arc[0].ext)
        Arc1 = ArcGeo(Pa = Pn, Pe = arc[2].Pe, O = Ob, r = rb,
                      s_ang = Ob.norm_angle(Pn),
                      e_ang = Ob.norm_angle(arc[2].Pe), dir = arc[2].ext)
    ...
    def calc_Biarc_section(self, u_sect, max_tol):
        ...
        while(u < u_sect[-1] - min_u):
            ...
            Biarc = BiarcClass(PtsVec[-1][0], PtsVec[-1][1],
                               PtVec[0], PtVec[1], max_tol)
            ...
            if Biarc.shape == "Zero":
                ...
                if Biarc.check_biarc_fitting_tolerance(self.NURBS, max_tol,
                                                       u - cur_step, u):
                    PtsVec.append(PtVec)
                    BiarcCurve.append(Biarc)
            ...
            if step > 1000:
                raise ValueError, "Iterations above 1000 - reduce tolerance"
...
class ExamplesClass:
    ...
    def calc_nurbs_1(self):
        ...
        Nurbs = NURBSClass(degree=degree, Knots=Knots,
                           CPoints=CPoints, Weights=Weights)
