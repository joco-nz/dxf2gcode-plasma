...
from matplotlib.numpy import arange, sin, pi
...
class ClassLine:
    ...
    def __str__(self):
        return ("\nLine Beg: %s; End: %s; Length %2.2f" % (self.beg, self.end, self.lenght))
...
class ClassPoint:
    ...
    def random_enpoint(self, start=None, max_len=5):
        from random import random
        self.x = start.x + (random() - 0.5) * 2 * max_len
        self.y = start.y + (random() - 0.5) * 2 * max_len
...
class ClassFittness:
    ...
    def __str__(self):
        return ("\nBest Fittness: %s \nBest Route: %s \nBest Pop: %s" \
                % (self.best_fittness[-1], self.best_route,
                self.population.pop[self.best_route]))
...
class ClassIteration:
    def __init__(self, master=None):
        ...
        self.Lines.random_lines(num=self.line_nrs, max_pos=self.line_max_pos,
                                max_len=self.line_max_len)
        ...
        self.Population.ini_random_pop(size = [self.line_nrs, self.pop_nr],
                                       dmatrix=self.DistanceMatrix.matrix)

        #Initialisieren der Result Class
        self.Fittness = ClassFittness(population=self.Population,
    ...
    def calc_next_iteration(self):
        ...
        self.Population.genetic_algorithm(Result=self.Fittness,
