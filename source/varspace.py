...
class VarSpace(object):
    ...
    def _cleanup(self, save=True, remove=False):
        ...
        if save:
            self._save_varspace()
            g.logger.debug('varspace %s saved' % (self.instance_name))
        if remove:
            os.remove(self.pathname)
            g.logger.info('varspace %s deleted' % (self.instance_name))

        self.top_nbook.hide(self.param_frame)
    ...
    def load_varspace(self, specversion):
        ...
        if os.path.isfile(self.pathname):
            try:
                ...
                if validate_errors:
                    g.logger.error("errors reading %s:" % (self.pathname))
                for entry in validate_errors:
                    ...
                    g.logger.error(section_string + ' = ' + error)       
                ...
                g.logger.debug("trying to rename bad cfg %s to %s" % (self.pathname, badfilename))
                try:
                    os.rename(self.pathname, badfilename)
                except OSError, e:
                    g.logger.error("rename(%s,%s) failed: %s" % (self.pathname, badfilename, e.strerror))
                    raise
                else:
                    g.logger.debug("renamed bad varspace %s to '%s'" % (self.pathname, badfilename))
                    self.create_default_varspace()
                    self.default_config = True
                    g.logger.debug("created default varspace '%s'" % (self.pathname))
            else:
                self.default_config = False
                g.logger.debug("read existing varspace '%s'" % (self.pathname))
        else:
            self.create_default_varspace()
            self.default_config = True
            g.logger.debug("created default varspace '%s'" % (self.pathname))
    ...
    def clone_instance(self):
        ...
        p.top_nbook = self.top_nbook
        p.window = self.window


        varspace_subdir = os.path.join(g.config.varspaces_dir, p.module.__name__)        
        p.instance_name = self.generate_valid_instance_name(p.module.__name__, varspace_subdir)
        
        if p.instance_name == None:
            g.logger.warning("clone_instance() failed - couldnt generate valid instance name for '%s', vs=%s" 
            ...
            if p.startup_plugin(vs_path): #,self.window,default=False):
                ...
                g.logger.debug("plugin %s new instance %s created" % (self.module.__name__, p.instance_name))
                return True
            else:
                g.logger.warning("skipping plugin %s instance %s: initialize() failed" 
    ...
    def close_instance(self, instance_name):
        ...
        g.logger.debug("plugin %s instance %s closed" % (p.module.__name__, instance_name))
    ...
    def delete_instance(self, instance_name):
        ...
        del(g.plugins[instance_name])
        g.logger.debug("plugin %s instance %s deleted" % (p.module.__name__, instance_name))
    ...
    def rename_instance(self, old_instancename, new_instancename):
        ...
        self.window.rebuild_export_menu()
        g.logger.debug("instance %s renamed to %s" % (old_instancename, new_instancename))
    ...
    def startup_plugin(self, varspace_path): #,window): #,default=False):   
        ...
        self.visible.set(self.VISIBLE_AT_STARTUP)
        self.default_config = False # wether a new name was generated

        #g.logger.debug("varspace %s visible=%s" % (self.instance_name,self.visible.get()))     
        ...
        try:
            ...
            g.logger.warning("varspace %s loading failed: %s" % (self.instance_name, msg))      
            return False
        ...
        if not _result and self.default_config:
            ...
            #del(p)
            g.logger.debug("cleanup plugin %s after failed initialize()", self.instance_name)      

        return _result


    def add_callback(self, varname, method):

        if self.tkVars.has_key(varname):
            self.tkVars[varname]._cb = method
        else:
            g.logger.warning("plugin %s - add_callback: no such variable '%s'", self.instance_name, varname)        
            
    def _uivar_changed_callback(self, varname, vdict, name, index, mode):
        ...
        try:
            ...
            #g.logger.debug("%s changed from %s to %s" %(varname,vdict[varname],newvalue))
            vdict[varname] = newvalue

            if hasattr(self.tkVars[varname], '_cb'):
                self.tkVars[varname]._cb(varname, oldvalue, newvalue)
            vdict[varname] = newvalue
    ...
    def save_callback(self):
        if self.is_default:
            self._save_varspace()
            g.logger.debug("varspace %s saved (default) " % (self.instance_name))  
        else:         
            new_name = self.tk_instance_name.get()
            if new_name != self.instance_name:
                
                if g.plugins.has_key(new_name): # uh-oh
                    g.logger.error("can't save to %s - instance already exists" % (new_name))
                else:
                    ...
                    # TODO robustify + log
                    os.remove(old_path)
                    self.top_nbook.rename(self.param_frame, self.instance_name)
                    
                    # fixup menu entries
                    self.rename_instance(old_instance, self.instance_name)
                    
                    # name labelframe
                    self._set_frame_name()

            else:
                self._save_varspace()
                g.logger.debug("varspace %s saved" % (self.instance_name))

    def _set_frame_name(self):
        if self.is_default:
    ...
    def create_pane(self):
        self.param_frame = Frame(self.top_nbook())
        self.groupcount = 0
    ...
    def display_pane(self, tab_name):
        self.top_nbook.add_screen(self.param_frame, tab_name) # ,highlightcolor='blue',disabledforeground='yellow')
        self.top_nbook.display(self.param_frame)    # switch to it

        if  not self.visible.get():                 # FIXME
            self.top_nbook.hide(self.param_frame)   
    ...
    def _add_item(self, frame, name, value, text, line, vdict):
        ...
        if  isinstance(value, float):
            ...
            self.tkVars[name].trace_variable("w", SimpleCallback(self._uivar_changed_callback, name, vdict))
    ...
    def _add_config_items(self, frame, name=None, value=None, line=0):
        ...
        if isinstance(value, dict): 
            ...
            self._add_item(frame, name, self.var_dict[c.VARIABLES][name], value, line, self.var_dict[c.VARIABLES])
...
class PluginLoader(object):
    ...
    def __init__(self, window, nbook):
        """
        ...
        @param window: the parameter notebook window
        """ 
        self.modules = dict()
        """ key: module basename, value: loaded module"""
        self.window = window
        self.top_nbook = nbook
    ...
    def activate(self, plugin_dir, base_name, cfg_path):
        ...
        if self.modules.has_key(base_name):
            g.logger.debug("module %s already loaded" % (base_name))
        ...
        if g.plugins.has_key(instance_name):
            g.logger.error("plugin instance %s already exists" % (instance_name))
        ...
        _instance.top_nbook = self.top_nbook
    ...
    def activate_all(self, plugin_dir, varspaces_dir):
        ...
        if len(_file_names) > 0:
            ...
            g.logger.warning("no plugins found in %s" % (plugin_dir))      
    ...
    def _import_plugin(self, plugin_dir, base_name):
        ...
        if not self.modules.has_key(base_name):
            ...
            try:
                ...
                g.logger.debug("module '%s' imported OK", _module.__name__)
        else:
            _module = self.modules[base_name]
        ...
        try:
            ...
            for _m in c.REQUIRED:
                ...
                g.logger.debug("module '%s' v='%s' validated" % (_module.__name__, _instance.VERSION))
